...
function formatValue(value, recurseTimes, seen, opts){
    ...
    seen.pop();
    return reduceToSingleString(output, braces, opts.breakLength);
};
...
function reduceToSingleString(output, braces, breakLength){
    var length = output.reduce(function(prev, cur){
        return prev + cur.replace(/\u001b\[\d\d?m/g, '').length + 1;
    });
    var separator = length > breakLength ? `\n${opts.indent}` : ' ';
    
    braces.splice(1, 0, output.join(`,${separator}`));
    return braces.join(separator);
};
